// Components
import { ReactComponent as JobAsusu } from "../assets/images/job-asusu.svg";
import { ReactComponent as JobBongalo } from "../assets/images/job-bongalo.svg";
import { ReactComponent as JobFreterium } from "../assets/images/job-freterium.svg";
import { NigeriaIcon } from "../components/footer/footer.styles";

// Project Items
export const PROJECT_ITEMS = [
  {
    id: 1,
    companyName: "Freterium",
    jobContext: "Redefining UX for Digital Transportation software",
    jobContextDescription:
      "As the lead UX designer on this project, I was involved in every aspect of its development, from initial wireframes to final handoff. Through our collaborative efforts, we were able to achieve a 45% increase in product usage, which was a significant milestone for the team.",
    Image: JobFreterium,
  },
  {
    id: 2,
    companyName: "Bongalo",
    jobContext:
      "Redesigning Bongalo: A temporary residency reservation product.",
    jobContextDescription:
      "As the lead product designer on this project, I Carefully analyzed and implemented UX Enhancements Doubled the online Booking to 65% in just 6 months. Organic search traffic increased by 2.5x and Increased monthly signups by 1500 users.",
    Image: JobBongalo,
  },
  {
    id: 3,
    companyName: "Asusu",
    jobContext: "Digitizing Savings and Credit Cooperatives (SACCOs)",
    jobContextDescription:
      "I had the privilege of being an early member of the Asusu team, where I played a key role in designing foundational features and products. One of my proudest achievements was re-imagining the web experience for Tellers in Credit Unions/Cooperatives, which helped streamline their workflow and improve their overall productivity.",
    Image: JobAsusu,
  },
];

// Career Highlight Items
export const CAREER_HIGHLIGHT_ITEMS = [
  {
    id: 1,
    content:
      "Migrated the design system from Sketch to Figma in record time for a leading US-based Healthcare product",
  },
  {
    id: 2,
    content:
      "I helped <strong>Freterium</strong> platform Double its Revenue & Save Time and Effort by optimizing workflows.",
  },
  {
    id: 3,
    content: "Recognised as ‘Top 1% Super Mentors’ on ADPList in March 2023",
  },
  {
    id: 4,
    content: "Designed 15+ Mobile & Web Apps in the last 6+ years",
  },
];

// Experience Items
export const EXPERIENCE_ITEMS = [
  {
    id: 1,
    jobRole: "Senior UX Designer",
    companyName: "Freterium",
    companyWebsite: "https://www.freterium.com/",
    companyExitDate: "March 2023",
  },
  {
    id: 2,
    jobRole: "Design Educator",
    companyName: "Verdac tech",
    companyWebsite: "https://verdac.tech/",
  },
  {
    id: 3,
    jobRole: "Design Mentor",
    companyName: "ADPlist",
    companyWebsite: "https://adplist.org/mentors/lukman-suleiman",
  },
  {
    id: 4,
    jobRole: "Lead Product Designer",
    companyName: "Bongalo",
    companyWebsite: "https://www.bongalo.com/",
  },
  {
    id: 5,
    jobRole: "Product Designer",
    companyName: "Agenpo",
    companyWebsite: "https://www.agenpo.com/home",
  },
  {
    id: 6,
    jobRole: "Senior UX/UI Designer",
    companyName: "Icecommpower",
    companyWebsite: "https://www.linkedin.com/company/icecommpower/",
  },
  {
    id: 7,
    jobRole: "Co-founder (Lead Designer)",
    companyName: "2B Design",
    companyWebsite: "https://www.instagram.com/2bdesignz/",
  },
  {
    id: 8,
    jobRole: "Founding Designer",
    companyName: "Asusu",
    companyWebsite: "https://www.asusu.ng/",
  },
];

// Reading Items
export const READING_ITEMS = [
  {
    id: 1,
    content: "The SEER Framework: For user-informed product development",
    link: "https://sprig.com/seer-framework#evaluate",
  },
  {
    id: 2,
    content:
      "Why New Users Should Discover Your Product’s “A-ha!” Moments Quickly",
    link: "https://productcraft.com/best-practices/first-impressions-matter-why-new-users-should-discover-your-products-a-ha-moments-quickly/",
  },
  {
    id: 3,
    content: "A Tried and True Framework for Prioritising User Research",
    link: "https://productcraft.com/best-practices/a-tried-and-true-framework-for-prioritizing-user-research/",
  },
  {
    id: 4,
    content:
      "Designing with data: Build trusted recommendations through this simple framework",
    link: "https://www.technicallyspeakinghw.com/articles/designing-with-data-build-trusted-recommendations-through-this-simple-framework",
  },
  {
    id: 5,
    content: "7 strategies to build a successful Design team",
    link: "https://www.technicallyspeakinghw.com/articles/7-strategies-to-build-a-successful-design-team",
  },
];

// Contact Items
export const CONTACT_ITEMS = [
  {
    id: 1,
    icon: "📧",
    content:
      "Email me on <a target='_blank' href='mailto:lukmann.design@gmail.com' rel='noopener noreferrer'>lukmann.design@gmail.com</a>",
  },
  {
    id: 2,
    icon: "🔗",
    content:
      "Connect with me on <a target='_blank' href='https://www.linkedin.com/in/lukmansuleiman/' rel='noopener noreferrer'>LinkedIn</a>",
  },
  {
    id: 3,
    icon: "📸",
    content:
      "Follow me on <a target='_blank' href='https://www.instagram.com/lukmansuleiman_/' rel='noopener noreferrer'>Instagram</a>",
  },
  {
    id: 4,
    icon: "👨🏾‍💻",
    content:
      "Download my <a target='_blank' href='https://drive.google.com/file/d/1cZpL7mV2dXzCU4xIWnzGh7zHEKXg9cOz/view' rel='noopener noreferrer'>Resume</a>",
  },
  {
    id: 5,
    icon: "📍",
    content: "Currently based in Nigeria",
    ContentIcon: NigeriaIcon,
  },
];

// Resources Items
export const RESOURCES_ITEMS = [
  {
    id: 1,
    content:
      "3 Persona Types: Lightweight, Qualitative, and Statistical by NNGroup",
    link: "https://www.nngroup.com/articles/persona-types/",
  },
  {
    id: 2,
    content:
      "5 things UX Researchers can do differently: a reflection after Uber’s layoffs by Elsa Ho",
    link: "https://uxdesign.cc/five-things-ux-researchers-can-do-differently-a-reflection-after-ubers-lay-off-9dd967148056",
  },
  {
    id: 3,
    content: "Complex Application Design: A 5-Layer Framework by NNGroup",
    link: "https://www.nngroup.com/articles/complex-application-design-framework/",
  },
  {
    id: 4,
    content: "Choosing Your Journey by MJ Blehart",
    link: "https://medium.com/the-ascent/choosing-your-journey-1eaf83a31921",
  },
  {
    id: 5,
    content:
      "A Customer Journey Map of the ProductPlan First Time User Experience by Jeremy Rawson",
    link: "https://www.productplan.com/customer-journey-map-first-time-user-experience/",
  },
  {
    id: 6,
    content: "Describing Personas by Indi Young",
    link: "https://medium.com/inclusive-software/describing-personas-af992e3fc527",
  },
  {
    id: 7,
    content: "Design’s Unsexy Middle Bits by Christina Wodtke",
    link: "https://cwodtke.medium.com/designs-unsexy-middle-bits-a8cc17f0246d",
  },
  {
    id: 8,
    content: "Designing emotional micro-interactions by Antony Ribot",
    link: "https://ribot.co.uk/stories/designing-emotional-micro-interactions",
  },
  {
    id: 9,
    content: "Designing Services That Deliver by G. Lynn Shostack",
    link: "https://hbr.org/1984/01/designing-services-that-deliver",
  },
  {
    id: 10,
    content:
      "Digital Transformation Is Not About Technology by Behnam Tabrizi , Ed Lam , Kirk Girard and Vernon Irvin",
    link: "https://hbr.org/2019/03/digital-transformation-is-not-about-technology",
  },
  {
    id: 11,
    content:
      "Escape Velocity: Finding Our Path in the Next Decade by Molly Stevens",
    link: "https://medium.com/@mollystevens/escape-velocity-finding-our-path-in-the-next-decade-2fb2a3846c1e",
  },
  {
    id: 12,
    content: "Five Habits of Design Thinking by Christina Wodtke",
    link: "http://eleganthack.com/five-habits-of-design-thinking-2/",
  },
  {
    id: 13,
    content:
      "Foundations to build on: Intercom’s principles for building product by Des Taylor",
    link: "https://www.intercom.com/blog/intercom-product-principles/",
  },
  {
    id: 14,
    content: "Generating Focused Ideas Through Practical Empathy by Indi Young",
    link: "https://medium.com/inclusive-software/generating-focused-ideas-through-practical-empathy-7a31092bf0f9",
  },
  {
    id: 15,
    content: "Good writing is good product design by Jordan Jackson",
    link: "https://foundationsoform.substack.com/p/good-writing-is-good-product-design",
  },
  {
    id: 16,
    content: "Hazard Mapping by Erika Hall",
    link: "https://medium.com/mule-design/hazard-mapping-e0b99b7ebd29",
  },
  {
    id: 17,
    content:
      "How to partner with UX researchers to positively impact product and business by Tatiana Vlahovic",
    link: "https://uxdesign.cc/how-to-partner-with-user-experience-researchers-to-positively-impact-product-and-business-a82580fc6ea5",
  },
  {
    id: 18,
    content: "How to Wield Empathy by Indi Young",
    link: "https://medium.com/inclusive-software/describing-personas-af992e3fc527",
  },
  {
    id: 19,
    content:
      "If time travel were possible, what would I tell myself as a junior UX researcher? by Mark",
    link: "https://medium.com/cheggux/if-time-travel-were-possible-what-would-i-tell-myself-as-a-junior-ux-researcher-30c5dfd7dc70",
  },
  {
    id: 20,
    content: "Jobs to be done personas by Nikki Anderson",
    link: "https://uxdesign.cc/jobs-to-be-done-personas-5faa0ea3d438",
  },
  {
    id: 21,
    content:
      "Jobs To Be Done: An Occasionally Useful UX Gimmick by Jared Spool",
    link: "https://articles.uie.com/jobs-to-be-done-an-occasionally-useful-ux-gimmick/",
  },
  {
    id: 22,
    content:
      "Know Your Customers’ “Jobs to Be Done” by Clayton M. Christensen, Taddy Hal, Karen Dillon and David S. Duncan",
    link: "https://hbr.org/2016/09/know-your-customers-jobs-to-be-done",
  },
  {
    id: 23,
    content:
      "The Ultimate Guide to Mixed Methods User Research by Ben Wiedmaier",
    link: "https://dscout.com/people-nerds/mixed-methods-research",
  },
  {
    id: 24,
    content: "My UX Research toolbox as a team of one by Lindsay Boylan",
    link: "https://uxdesign.cc/my-ux-research-toolbox-as-a-team-of-one-a7aa5041051d",
  },
  {
    id: 25,
    content: "Needfinding for Disruptive Innovation by Christina Wotdke",
    link: "https://medium.com/the-creative-founder/needfinding-for-disruptive-innovation-71d8532f2cf3",
  },
  {
    id: 26,
    content: "Product People, Mind the Gap! by Samuel Hulick",
    link: "https://ux.useronboard.com/product-people-mind-the-gap-da363018cc57",
  },
  {
    id: 27,
    content: "Research Questions Are Not Interview Questions by Erika Hall",
    link: "https://medium.com/mule-design/research-questions-are-not-interview-questions-7f90602eb533",
  },
  {
    id: 28,
    content:
      "Console Gaming Then and Now – 1997 Developer Interview by Gunpei Yokoi x Yukihito Morikawa",
    link: "http://shmuplations.com/yokoi/",
  },
  {
    id: 29,
    content: "Six Chasms in Need of Crossing by Anirudh Dhebar",
    link: "https://sloanreview.mit.edu/article/six-chasms-in-need-of-crossing/",
  },
  {
    id: 30,
    content: "Six Questions to Structure Great Projects by J.T. Trollman",
    link: "https://medium.com/facebook-design/six-questions-to-structure-great-projects-24a557cb11ab",
  },
  {
    id: 31,
    content: "Surviving enterprise UX: communicating value by Lisa Angela",
    link: "https://uxdesign.cc/surviving-enterprise-ux-part-1-6c37cdddf745",
  },
  {
    id: 32,
    content:
      "Surviving Enterprise UX: the importance of targeted user research by Lisa Angela",
    link: "https://uxdesign.cc/surviving-enterprise-ux-the-importance-of-targeted-user-research-c9823568ef1f",
  },
  {
    id: 33,
    content: "The 7 Deadly Sins of User Research by David Travis",
    link: "https://www.userfocus.co.uk/articles/7-sins-of-user-research.html",
  },
  {
    id: 34,
    content: "The 9 Rules of Design Research by Erika Hall",
    link: "https://medium.com/mule-design/the-9-rules-of-design-research-1a273fdd1d3b",
  },
  {
    id: 35,
    content: "The Complete Guide to the Kano Model by Folding Burritos",
    link: "https://foldingburritos.com/kano-model/",
  },
  {
    id: 36,
    content:
      "The Emotional Machine: Searching for Soul in the Autonomous Future",
    link: "https://medium.com/dford/the-emotional-machine-9de128e7af0f",
  },
  {
    id: 37,
    content: "The Future of UX Research by Monty Hammontree",
    link: "https://medium.com/microsoft-design/the-future-of-ux-research-14fe63743c1d",
  },
  {
    id: 38,
    content: "The History of Design Anthropology by Matt Artz",
    link: "https://medium.com/@matt_artz_anthropology/the-history-of-design-anthropology-ad04ce2792a",
  },
  {
    id: 39,
    content:
      "The lost art of injecting humanity into product design by Richard Bailey",
    link: "https://thenextweb.com/podium/2019/09/30/the-lost-art-of-injecting-humanity-into-product-design/",
  },
  {
    id: 40,
    content:
      "The New Enterprise Designer by Craig Mackintosh, American Express and Linda Quarles, frog",
    link: "https://www.frogdesign.com/designmind/the-new-enterprise-designer",
  },
  {
    id: 41,
    content: "The Next Century of Design Education by Albert Shum",
    link: "https://modus.medium.com/the-case-for-meta-modernist-design-5ca38ee7cc9d",
  },
  {
    id: 42,
    content: "The Pyramid Principle by Ameet Ranadive",
    link: "https://medium.com/lessons-from-mckinsey/the-pyramid-principle-f0885dd3c5c7",
  },
  {
    id: 43,
    content:
      "The Quintessential Guide For Building An Unforgettable First-time User Experience by Wayne Chang",
    link: "https://hackernoon.com/the-quintessential-guide-for-building-an-unforgettable-first-time-user-experience-19720a7447d2",
  },
  {
    id: 44,
    content:
      "The Top 5 Behavioural Economics Principles for Designers by Bridgeable",
    link: "https://uxplanet.org/the-top-5-behavioural-economics-principles-for-designers-ea22a16a4020",
  },
  {
    id: 45,
    content: "The Ultimate Guide to Customer Onboarding by Christina Perricone",
    link: "https://blog.hubspot.com/service/customer-onboarding",
  },
  {
    id: 46,
    content:
      "Uncover and make the most of your signature moments by Jess Leitch",
    link: "https://www.idean.com/spotlight/making-the-most-of-your-signature-moments",
  },
  {
    id: 47,
    content:
      "Understand and define users with the User Profile Model by Jessica Sherratt",
    link: "https://uxdesign.cc/understand-and-define-users-with-the-user-profile-model-f242047faaa9",
  },
  {
    id: 48,
    content:
      "Understanding your “aha” moments and putting them to work by Lynsey Duncan",
    link: "https://www.intercom.com/blog/understanding-your-aha-moments-and-putting-them-to-work/",
  },
  {
    id: 49,
    content: "User research is a team sport by Myddelton",
    link: "https://www.myddelton.co.uk/blog/user-research-is-a-team-sport",
  },
  {
    id: 50,
    content: "What Autonomy Can Deliver by Jonah Houston",
    link: "https://medium.com/dford/what-autonomy-delivers-8e4b9a38bb61",
  },
  {
    id: 51,
    content: "What UX from 1989 can teach us by Dina Zuko",
    link: "https://uxdesign.cc/what-ux-from-1989-can-teach-us-22476ac5703c",
  },
  {
    id: 52,
    content: "When Coffee and Kale Compete by Vipul Bansal",
    link: "https://medium.com/@vipsyvipul/when-coffee-and-kale-compete-45e1155c26f1",
  },
  {
    id: 53,
    content:
      "When Customers Want to See the Human Behind the Product by Adam Waytz",
    link: "https://hbr.org/2019/06/when-customers-want-to-see-the-human-behind-the-product",
  },
  {
    id: 54,
    content: "Why Highly Intelligent People Are Miserable by Michael Ndubuaku",
    link: "https://medium.com/swlh/why-highly-intelligent-people-are-miserable-c257f7d0680c",
  },
];

// Freterium Project Hero Details
export const FRETERIUM_PROJECT_HERO_DETAILS = {
  jobContext: "Redefining UX for Digital Transportation software",
  projectYear: 2023,
  projectWebsite: "https://www.freterium.com/",
  jobRole: "Lead UX designer",
  projectPlatform: [
    { id: 1, name: "Android App" },
    { id: 2, name: "Responsive Web App" },
  ],
  jobRoleSummary: [
    {
      id: 1,
      name: "Research",
    },
    {
      id: 2,
      name: "Leading Design Workshop",
    },
    {
      id: 3,
      name: "Wireframing",
    },
    {
      id: 4,
      name: "Screen Flows",
    },
    {
      id: 5,
      name: "UX Design",
    },
    {
      id: 6,
      name: "Visual Design",
    },
    {
      id: 7,
      name: "Prototyping",
    },
    {
      id: 8,
      name: "Usability Testing",
    },
  ],
  jobCollaborators: [
    {
      id: 1,
      collaboratorName: "Réda MANSOURI",
      collaboratorRole: "Head of prodcut",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/r%C3%A9da-mansouri-26015691/",
    },
    {
      id: 2,
      collaboratorName: "Reda SAKHI",
      collaboratorRole: "Product Owner",
      collaboratorSocialLink: "https://www.linkedin.com/in/redasakhi/",
    },
    {
      id: 3,
      collaboratorName: "Viktor Demydov",
      collaboratorRole: "UX/UI Designer",
      collaboratorSocialLink: "https://www.linkedin.com/in/vicd129/",
    },
    {
      id: 4,
      collaboratorName: "Ghita Dassouli",
      collaboratorRole: "Business Analyst & Customer Success Manager",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/ghita-dassouli-077878152/",
    },
    {
      id: 5,
      collaboratorName: "Almahdi Guedira",
      collaboratorRole: "Head of Customer Success",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/almahdi-guedira-5b9b236b/",
    },
    {
      id: 6,
      collaboratorName: "Denis Kurilov",
      collaboratorRole: "Tech Lead",
      collaboratorSocialLink: "https://www.linkedin.com/in/dkourilov/",
    },
  ],
};

// Bongalo Project Hero Details
export const BONGALO_PROJECT_HERO_DETAILS = {
  jobContext:
    "Bongalo (Africa’s Airbnb) is a platform that provides solution for finding vacation homes.",
  projectYear: 2021,
  projectWebsite: "https://bongalo.com/",
  jobRole: "Lead Product designer",
  projectPlatform: [
    {
      id: 1,
      name: "Android/ iOS App",
    },
    {
      id: 2,
      name: "Responsive Web App",
    },
  ],
  jobRoleSummary: [
    {
      id: 1,
      name: "User Interview",
    },
    {
      id: 2,
      name: "Wireframing",
    },
    {
      id: 3,
      name: "Screen Flows",
    },
    {
      id: 4,
      name: "Visual Design",
    },
    {
      id: 5,
      name: "Interaction Design",
    },
  ],
  jobCollaborators: [
    {
      id: 1,
      collaboratorName: "Jr Nosouh Tonteh",
      collaboratorRole: "Product Manager",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/jr-nosouh-tonteh-4052871ba/",
    },
    {
      id: 2,
      collaboratorName: "Nsikan Etukudoh",
      collaboratorRole: "Product designer",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/jr-nosouh-tonteh-4052871ba/",
    },
    {
      id: 3,
      collaboratorName: "David Atebisun",
      collaboratorRole: "Frontend engineer",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/david-atebisun-4ab786171/",
    },
    {
      id: 4,
      collaboratorName: "Nelson Bassey",
      collaboratorRole: "Lead Software engineer",
      collaboratorSocialLink: "https://www.linkedin.com/in/nbassey/",
    },
    {
      id: 5,
      collaboratorName: "Minuifuong Nghombombong",
      collaboratorRole: "CEO",
      collaboratorSocialLink:
        "https://www.linkedin.com/in/epaphrate-minuifoung?miniProfileUrn=urn%3Ali%3Afs_miniProfile%3AACoAABYbE28BlyRs7sOj2XPL42bcgdevjU6cjA8",
    },
  ],
};
